...
import { insertResumeSchema, insertAnalysisSchema, type AnalysisResult } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/resumes/upload", upload.array('resumes', 10), async (req, res) => {
    try {
      ...
      if (uploadedResumes.length === 0) {
        return res.status(400).json({ message: "No valid PDF files could be processed" });
      }
      ...
      res.status(500).json({ message: "Failed to upload resumes: " + (error as Error).message });
    }
  });
  ...
  app.post("/api/analyze", async (req, res) => {
    try {
      ...
      // Analyze with OpenAI
      const candidates = await analyzeResumes(jobDescription.trim(), resumesForAnalysis);
  ...
  app.get("/api/export/:format", async (req, res) => {
    try {
      ...
      if (!analysis) {
        return res.status(404).json({ message: "No analysis results to export" });
      }
      ...
      if (format === 'json') {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="resume-analysis.json"');
        res.json(results);
      } else {
        res.status(400).json({ message: "Unsupported export format. Use 'json'." });
      }
    } catch (error) {
