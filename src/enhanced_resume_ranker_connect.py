...
class UniversalResumeRanker:
    ...
    def process_resume_folder(self, data_path: str) -> List[ResumeData]:
        """Process all resumes in the data folder with enhanced error handling"""
        resumes_data = []
        ...
        logger.info(f"Processing resumes from {data_path}")
        
        for sector_folder in data_path.iterdir():
            if not sector_folder.is_dir():
                continue
                
            logger.info(f"Processing sector: {sector_folder.name}")
            sector_count = 0
            ...
            for pdf_file in sector_folder.glob("*.pdf"):
                try:
                    ...
                    if file_size > self.config['max_file_size_mb'] * 1024 * 1024:
                        logger.warning(f"Skipping {pdf_file.name}: File too large ({file_size/1024/1024:.1f}MB)")
                        continue
                    ...
                    if text and len(text.strip()) > 50:
                        ...
                        logger.warning(f"Insufficient text extracted from {pdf_file.name}")
                        
                except Exception as e:
                    logger.error(f"Error processing {pdf_file.name}: {e}")
                    continue
            
            logger.info(f"Processed {sector_count} resumes from {sector_folder.name}")
        
        logger.info(f"Total resumes processed: {len(resumes_data)}")
        return resumes_data
